...
def register_annotation_tools(mcp: FastMCP, conn_manager: ConnectionManager) -> None:
    ...
    @mcp.tool()
    async def get_table_annotations(table_name: str) -> str:
        ...
        Args:
            table_name: Name of the table (e.g., "Image", "Subject").

        Returns:
            JSON with annotation values for display, visible_columns,
            visible_foreign_keys, and table_display. Missing annotations
            return null.
        ...
        Example:
            get_table_annotations("Image") -> {
                ...
                "display": {"name": "Images"},
                "visible_columns": {"compact": ["RID", "Filename"], ...},
                "visible_foreign_keys": {"detailed": [["schema", "fkey"]]},
                "table_display": {"row_name": {"row_markdown_pattern": "..."}}
            }
        """
        try:
    ...
    @mcp.tool()
    async def get_column_annotations(table_name: str, column_name: str) -> str:
        ...
        Args:
            ...
            column_name: Name of the column (e.g., "Filename", "Description").

        Returns:
            JSON with annotation values for display and column_display.
            Missing annotations return null.

        Example:
            get_column_annotations("Image", "Filename") -> {
                "table": "Image",
                "column": "Filename",
                "display": {"name": "File Name"},
                "column_display": {"*": {"markdown_pattern": "..."}}
            }
        """
        try:
            ml = conn_manager.get_active_or_raise()
    ...
    @mcp.tool()
    async def list_foreign_keys(table_name: str) -> str:
        """List all foreign keys related to a table.

        Returns both outbound foreign keys (from this table to others) and
        inbound foreign keys (from other tables to this one). This is useful
        for determining valid constraint names to use in visible-columns and
        ...
        Returns:
            JSON with outbound and inbound foreign key lists, each containing:
            - constraint_name: [schema, constraint] array for use in annotations
        ...
        Example:
            list_foreign_keys("Image") -> {
                ...
                "outbound": [
                    {
                        "constraint_name": ["domain", "Image_Subject_fkey"],
                        "from_table": "Image",
                        "from_columns": ["Subject"],
                        "to_table": "Subject",
                        "to_columns": ["RID"]
                    }
                ],
                "inbound": [
                    {
                        "constraint_name": ["domain", "Diagnosis_Image_fkey"],
                        "from_table": "Diagnosis",
                        "from_columns": ["Image"],
                        "to_table": "Image",
                        "to_columns": ["RID"]
                    }
    ...
    @mcp.tool()
    async def get_annotation_contexts() -> str:
        """Get documentation about annotation contexts and their usage.

        Returns comprehensive documentation about all valid contexts for
        visible-columns, visible-foreign-keys, table-display, and column-display
        annotations.

        Returns:
            JSON with context documentation organized by annotation type.

        This is a reference tool - use it to understand which contexts are
        valid for each annotation type and when they are applied.
        """
        return json.dumps({
            "visible_columns_contexts": {
                ...
                "contexts": {
                    ...
                    "entry": {
                        ...
                        "usage": "Base configuration for all entry forms"
                    },
                    "entry/create": {
                        "description": "Create new record form only",
                        "usage": "Overrides 'entry' for new record creation"
                    },
                    "entry/edit": {
                        "description": "Edit existing record form only",
                        "usage": "Overrides 'entry' for editing existing records"
                    },
                    "export": {
                    ...
                    "filter": {
                        "description": "Faceted search configuration",
                        "usage": "Search sidebar facets (uses different format with 'and' array)"
                    }
                },
                "fallback_order": [
                    "specific context (e.g., 'compact/brief')",
                    "parent context (e.g., 'compact')",
                    "'*' (wildcard default)",
                    "system default (all columns)"
                ]
            },
            "visible_foreign_keys_contexts": {
                "description": "Contexts control which related tables appear",
                "contexts": {
                    "*": {
                        "description": "Default for all contexts",
                        "usage": "Fallback when specific context not defined"
                    },
                    "detailed": {
                        "description": "Full record view",
                        "usage": "Related tables shown on single record page"
                    }
                },
                "note": "Only inbound foreign keys (tables referencing this table) are valid"
            },
            "table_display_contexts": {
                "description": "Contexts control table-level display options",
                "contexts": {
                    "*": {
                        "description": "Default for all contexts",
                        "options": ["row_order", "page_size"]
                    },
                    "row_name": {
                        "description": "Row identifier display",
                        "options": ["row_markdown_pattern", "template_engine"],
                        "usage": "How rows are identified in links, breadcrumbs, titles"
                    },
                    "compact": {
                        "description": "List view options",
                        "options": ["row_order", "page_size", "row_markdown_pattern"]
                    },
                    "detailed": {
                        "description": "Record view options",
                        "options": ["hide_column_headers", "collapse_toc_panel", "page_markdown_pattern"]
                    }
                }
            },
            "column_display_contexts": {
                "description": "Contexts control column value rendering",
                "contexts": {
                    "*": {
                        "description": "Default for all contexts",
                        "options": ["pre_format", "markdown_pattern", "template_engine", "column_order"]
                    },
                    "compact": {
                        "description": "List view rendering",
                        "usage": "How values appear in table cells"
                    },
                    "detailed": {
                        "description": "Record view rendering",
                        "usage": "How values appear on record page"
                    },
                    "entry": {
                        "description": "Entry form rendering",
                        "usage": "Display during data entry (usually not customized)"
                    }
                }
            },
            "column_directive_formats": {
                "description": "Ways to specify columns in visible-columns lists",
                "formats": {
                    "string": {
                        "example": "\"Filename\"",
                        "description": "Simple column name from the table"
                    },
                    "foreign_key_array": {
                        "example": "[\"schema_name\", \"constraint_name\"]",
                        "description": "Reference to a foreign key constraint",
                        "note": "Shows the related entity value"
                    },
                    "pseudo_column_object": {
                        "example": {
                            "source": "column_or_path",
                            "markdown_name": "Display Name",
                            "entity": True,
                            "display": {"markdown_pattern": "..."}
                        },
                        "description": "Complex column with custom source path and display"
                    }
                }
            },
            "examples": {
                "common_patterns": [
                    {
                        "goal": "Show only key columns in list view, all columns in detail",
                        "annotation": {
                            "compact": ["Name", "Status", "Created"],
                            "detailed": ["Name", "Status", "Description", "Notes", "Created", "Modified"]
                        }
                    },
                    {
                        "goal": "Hide system columns in entry forms",
                        "annotation": {
                            "entry": ["Name", "Description", "Status"],
                            "*": ["RID", "Name", "Description", "Status", "RCT", "RMT"]
                        }
                    },
                    {
                        "goal": "Show related entity name instead of ID",
                        "annotation": {
                            "compact": [
                                "Name",
                                {"source": [{"outbound": ["schema", "Table_Parent_fkey"]}, "Name"],
                                 "markdown_name": "Parent"}
                            ]
                        }
                    }
                ]
