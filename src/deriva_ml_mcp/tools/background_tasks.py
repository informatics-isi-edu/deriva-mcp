...
Clone operations use the production create_ml_workspace implementation
from deriva_ml.catalog, which provides:
- Three-stage clone: create schema without FKs, copy data, apply FKs
- Proper orphan handling (fail/delete/nullify strategies)
- Async data copying via export paths + fill-in tables
- Detailed CloneCatalogResult reporting
- Export annotation support for partial clones
"""
...
def _clone_catalog_task(
    ...
    Uses create_ml_workspace for partial cloning from a root RID.
    """
...
def register_background_task_tools(mcp: FastMCP, conn_manager: ConnectionManager) -> None:
    ...
    async def clone_catalog_async(
        ...
        Starts the workspace creation in the background and immediately returns
        a task_id that you can use to check progress.

        The operation uses export annotations (if available) to determine which
        tables and paths to follow from the root RID, then fills in any uncovered
        tables (vocabularies, associations). Uses async data copying for performance.
        ...
        Args:
            ...
            root_rid: The starting RID from which to trace reachability (e.g., "3-HXMC").
