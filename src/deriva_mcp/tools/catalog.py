...
def register_catalog_tools(mcp: FastMCP, conn_manager: ConnectionManager) -> None:
    ...
    async def connect_catalog(
        ...
        try:
            ...
            # Write .deriva-context.json for script handoff
...
def _write_context_file(
    ...
    """Write .deriva-context.json to cwd for script handoff.

    This file allows Python scripts generated by Claude to create a
    DerivaML instance via ``DerivaML.from_context()`` without hardcoding
    connection details.
    """
    import os
    ...
    context_path = Path(os.getcwd()) / CONTEXT_FILENAME
    context_path.write_text(json.dumps(context, indent=2))
    logger.info(f"Wrote context file: {context_path}")


def _remove_context_file() -> None:
    """Remove .deriva-context.json from cwd."""
    import os

    context_path = Path(os.getcwd()) / CONTEXT_FILENAME
    if context_path.exists():
        context_path.unlink()
        logger.info(f"Removed context file: {context_path}")
