...
- `~/.deriva:~/.deriva:ro` - Mounts your Deriva credentials (read-only), at the same path in the container
- `~/.bdbag:~/.bdbag` - Mounts bdbag keychain for dataset download authentication (writable)
